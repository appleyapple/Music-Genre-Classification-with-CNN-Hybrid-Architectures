...
def build_crnn_model_regular():
    ...
    KERNAL_SIZE = [(11,11), (7,7), (3,3), (3, 3), (3,3), (3,3), (2,2)]
    POOL = [(2, 2), (2,2), (2,2), (2,2), (2,2), (2,2), (2,2), (2,2), (2,2)]
    ACTIVATION = 'relu'
    NUM_CONV_LAYERS = 3
    FILTERS = [4, 8, 16, 32, 64, 128, 512]
    ...
    model.compile(optimizers.Adam(lr=0.0005),
...
def build_cnn_model_regular():
    ...
    for layer in range(NUM_CONV_LAYERS - 1):
        model.add(Conv2D(FILTERS[layer + 1], KERNAL_SIZE[layer + 1], padding='same'))
        model.add(Activation(ACTIVATION))
        model.add(MaxPooling2D(pool_size=POOL))
        model.add(Dropout(0.5))
...
def build_cnn_model_duplicated():
    KERNAL_SIZE = [(11, 11), (7, 7), (5, 5), (4, 4), (4, 2),
                   (2, 4), (3, 3), (3, 3), (3, 3)]
    POOL = [(11, 11), (5, 5), (7, 7), (2, 2), (2, 2),
            (2, 2), (2, 2), (2, 2), (2, 2)]
    ACTIVATION = 'relu'
    NUM_CONV_LAYERS = 4
    FILTERS = [4, 4, 8, 64, 128, 256, 256]
    ...
    # Output layer
    # L5 = Dense(128, activation=ACTIVATION, kernel_regularizer=regularizers.l2(0.003))(flattened)
    A4 = Dense(128, activation=ACTIVATION )(flattened)
    L7 = Dropout(0.3)(A4)
    L8 = Dense(8, activation='softmax')(L7)

    model = Model(inputs=L1, outputs=L8)

    model.compile(optimizers.Adam(lr=0.0002),
                  loss="categorical_crossentropy", metrics=["accuracy"])
    model.summary()
...
def build_crnn_model_duplicated():
    ...
    POOL = [(2, 2), (2, 2), (2, 2), (2, 2), (2, 2),
            (2, 2), (2, 2), (2, 2), (2, 2)]
    ACTIVATION = 'relu'
    FILTERS = [4, 4, 8, 32, 128, 256, 256]

    # Input layer
    L1 = Input(shape=INPUT_SHAPE, name="input")
    B = BatchNormalization()(L1)
    # Visible layers
    L2 = Conv2D(FILTERS[0], KERNAL_SIZE[0],
                padding='same', activation=ACTIVATION, kernel_regularizer=regularizers.l2(0.02))(B)
    L3 = Dropout(0.2)(L2)
    L4 = Conv2D(FILTERS[1], KERNAL_SIZE[1],
                padding='same', activation=ACTIVATION, kernel_regularizer=regularizers.l2(0.01))(L3)
    ...
    # Recurrent layer
    R3 = GRU(64, return_sequences=True, dropout=0.5)(R2)
    R4 = GRU(64, return_sequences=False, dropout=0.5)(R3)
    # R5 = Dropout(0.1)(R4)
